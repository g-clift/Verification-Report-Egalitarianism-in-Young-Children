...
#Select Variables for Proportion Analysis
...
#write csv file of this data table
write.csv(egalbinomtable, here::here("results", "fehr_egalbinomtable.csv"))

#Prepare Graph Data
#create data frame with proportion estimates, egalitarian & non-egalitarian frequencies and N.
graphdata <-data.frame(dplyr::select(egalbinomtable, AgeClass, GroupStat, GameType, NonEgalitarian, Egalitarian, N, estimate))

#use tidyverse to create data subset with only Prosocial GameType
pvgraphdata<- filter(graphdata, GameType == "Prosocial_Num")

#use tidyverse to create data subset with only Envy GameType
evgraphdata<- filter(graphdata, GameType == "Envy_Num")

#use tidyverse to create data subset with only Sharing GameType
svgraphdata<- filter(graphdata, GameType == "Share_Num")
...
ppgraph <- ggplot(pvgraphdata,
  ...
  # Specify axis labels
  labs(x = "Age Group", y = "Proportion of Prosocial Choice", 
  ...
  scale_y_continuous(name = "Proportion of Prosocial Choice", limits = c(0, 1))
...
ggsave(filename = here::here("images", "prosocial_proportion_graph.png"), plot = ppgraph, width = 7, height = 7, dpi = 300)
...
epgraph <- ggplot(evgraphdata,
  ...
  # Set scale limits for y axis variable
  scale_y_continuous(name = "Proportion of Egalitarian Choice", limits = c(0, 1))
...
# Save graph as a .png file  
ggsave(filename = here::here("images", "envy_proportion_graph.png"), plot = epgraph, width = 7, height = 7, dpi = 300)
...
# Save graph as a .png file  
ggsave(filename = here::here("images", "sharing_proportion_graph.png"), plot = spgraph, width = 7, height = 7, dpi = 300)
