...
mv <- read_excel(here::here("processed_data", "processed_data.xlsx"))
...
# Create a list of glm models to compare to glm1
glm_list <- list(glm1, glm2, glm3, glm4, glm5, glm6)

# Create a tibble to store the results
modelanova <- tibble(glm_model = "",
                     deviance = 0,
                     df = 0,
                     p_value = 0)

# Loop through the indices of the glm models in glm_list
for (i in 2:length(glm_list)+1) {
  
  # Use anova() to compare glm1 to the current glm model in the loop
  anova_result <- anova(glm1, glm_list[[i-1]], test = "Chisq")
  
  # Store the results in the tibble
  modelanova <- modelanova %>% add_row(glm_model = paste0("glm", i-1),
                                       deviance = anova_result$Deviance[2],
                                       df = anova_result$Df[2],
                                       p_value = anova_result$Pr[2])
}

#observe null model statistics
anova1_2 <- as.data.frame(anova(glm1, test = "Chisq"))
...
#manipulapte to convert to dataframe
modelstats <- ldply(modelstats)

#transpose so each model is a different row
modelstats <- as.data.frame(t(modelstats))

#write csv file of 'modelstats' table
write.csv(modelstats, here::here("Results", "fehr_modelstats.csv"))
...
pprobgraph <- ggplot(data=pvplotdata2,
  ...
  # Specify axis labels
  labs(x="Age", y="Probability of Prosocial Choice", 
       
       # Specify graph title
       title="Probability of Prosocial Choice in Prosocial Mini-Game") + 
 
   # Add dotted horizontal line to represent chance performance at 50%
  geom_hline(yintercept = 0.50, size = 0.5, linetype = "dotted", alpha = 0.4) +
  
  # Set scale limits for y axis variable
  scale_y_continuous(name = "Probability of Prosocial Choice", limits = c(0, 1))

# Use jtools package to apply APA theme to graph
pprobgraph + theme_apa()

# Save graph as a .png file  
ggsave(here::here("images", "prosocial_probability_graph.png"), width = 7, height = 7, scale = 1, limitsize = FALSE)
...
eprobgraph <- ggplot(data=evplotdata2,
  ...
  labs(x="Age", y="Probability of Envy Choice", 
       title="Probability of Envy Choice in Envy Mini-Game") + 
  
  # Add dotted horizontal line to represent chance performance at 50%
  geom_hline(yintercept = 0.50, size = 0.5, linetype = "dotted", alpha = 0.4) +
  
  # Set scale limits for y axis variable
  scale_y_continuous(name = "Probability of Envy Choice", limits = c(0, 1))

# Use jtools package to apply APA theme to graph
eprobgraph + theme_apa()

# Save graph as a .png file  
ggsave(here::here("images", "envy_probability_graph.png"), width = 7, height = 7, dpi = 300, limitsize = FALSE)
...
# Create a data frame with the Age and GroupStat_Num predictors
svplotdata <- with(sv, data.frame(Age = rep(seq(from = 0, to = 10, length.out = 100), 2), 
                                  GroupStat_Num = factor(rep(0:1, each = 100))))

# Combine the svplotdata data frame with predicted values from the ms1 model
svplotdata2 <- cbind(svplotdata, 
                     predict(ms1, svplotdata, type = "link", se = TRUE))

# Mutate variables to the correct class
svplotdata2 <- svplotdata2 %>%
...
# Calculate the probability distribution of the ms1 model
svplotdata2 <- within(svplotdata2, {svprob <- plogis(fit)})

# Map the GroupStat_Num factor to In-Group and Out-Group labels
svplotdata2$Group <- as.factor(mapvalues(svplotdata2$GroupStat_Num, from = c("0", "1"), 
                                         to = c("Out-Group", "In-Group")))

# Create a table with the Group factor
table(svplotdata2$Group)

# Create a Sharing game Probability Plot
sprobgraph <- ggplot(data = svplotdata2, 
                     aes(x = Age, y = svprob)) + 
  
  # Define the linetype based on the Group factor
  geom_line(mapping = aes(linetype = Group), size = 1) +
  
  # Place the legend at the top of the graph
  theme(legend.position = "top") +
  
  # Define the axis labels and graph title
  labs(x = "Age", y = "Probability of Egalitarian Choice", 
       title = "Probability of Egalitarian Choice in Sharing Mini-Game") + 
  
  # Add dotted horizontal line to represent chance performance at 50%
  geom_hline(yintercept = 0.50, size = 0.5, linetype = "dotted", alpha = 0.4) +
  
  # Set the scale limits for the y-axis
  scale_y_continuous(name = "Probability of Egalitarian Choice", limits = c(0, 1))

# Use the jtools package to apply an APA theme to the graph
sprobgraph + theme_apa()

# Save the graph as a .png file in the "images" directory using the here package
ggsave(here::here("images", "sharing_probability_graph.png"), width = 7, height = 7, 
       scale = 1, limitsize = FALSE)


#save graph as a .png file  
ggsave(here::here("images", "sharing_probability_graph.png"), width = 7, height = 7, scale = 1, limitsize = FALSE)
